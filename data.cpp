   ...
   for(unsigned int j = 0; j < actual_block_size; j++)
   {
	 ...
	 double P, sd;
	    ...
	    if(stand_method_x == STANDARDISE_BINOM)
	       ...
	       sd = sqrt(2.0 * P * (1 - P));
	    else if(stand_method_x == STANDARDISE_NONE || stand_method_x == STANDARDISE_CENTER)
	       sd = 1;
	    else
	    {
	 ...
	 if(sd > VAR_TOL)
	 {
	    ...
	    //*                   plink BED           sparsnp
 	    ...
 	    //* major homozygous: 11 => numeric 3     00 => numeric 0
 	    //* missing:          01 => numeric 1     11 => numeric 3
 	    if (stand_method_x == STANDARDISE_NONE){
	      scaled_geno_lookup(3, k) = 0;
              scaled_geno_lookup(2, k) = 1;
              scaled_geno_lookup(0, k) = 2;
              scaled_geno_lookup(1, k) = snp_avg; // impute to average
            }
	    else {
...
void Data::read_plink_bim(const char *filename)
{
   ...
   verbose && STDOUT << timestamp() << "Finished reading BIM!" << std::endl;
}
...
verbose && STDOUT << timestamp() << "Detected fam file " <<
      filename << ", " << lines.size() << " individuals" << std::endl;
   in.close();
   ...
   verbose && STDOUT << timestamp() << "Finished reading FAM!" << std::endl;
}
