...
int main(int argc, char * argv[])
{
   ...
   desc.add_options()
	 ...
	 "standardization method for genotypes [binom | bern | none | center]")
   ...
   std::cout << timestamp() << "arguments: binomial_lse ";
   ...
   if(vm.count("version"))
   {
      std::cerr << "Binomial LSE " << VERSION << std::endl;
      std::cerr << "Copyright (C) 2014-2016 Gad Abraham." << std::endl
	 << "This is free software; see the source for copying conditions.  There is NO"
	 << std::endl
	 << "warranty; not even for MERCHANTABILITY or FITNESS FOR A PARTICULAR PURPOSE."
	 << std::endl << std::endl
    << "FlashPCA code modified by Alec Chiu to implement binomial latent subspace"
    << std::endl
    << "estimation as described by Chen and Storey 2015."
    << std::endl << std::endl;
      return EXIT_SUCCESS;
   }
   ...
   std::string fam_file, geno_file, bim_file;
   ...
   std::cout << timestamp() << "Start Binomial LSE (version " << VERSION
      << ")" << std::endl;
#ifdef _OPENMP
   ...
   try
   {
      ...
      verbose && std::cout << timestamp() << "About to get size" << std::endl;
      data.get_size();
      ...
      verbose && std::cout << timestamp() << "Finished preparing data" << std::endl; 
	 ...
	 long long mem_req_bytes =
       ...
       + 2 * (long long)(data.N) * 8 // lse adjustment
